...
task_groups = {
    'math': [
        'math_mini_14_final', 'math_mini_28_final',
        'math_mini_56_final', 'math_mlt_final'
    ],
    'facts': [
        'facts_mini_14_final', 'facts_mini_28_final',
        'facts_mini_56_final', 'facts_mlt_final'
    ],
    'combined': [
        'both_mini_14_final', 'both_mini_28_final',
        'both_mini_56_final', 'both_mlt_final'
    ]
}
...
def plot_group(root_dir, group_name, folders, output):
    ...
    fig.suptitle(group_titles.get(group_name, ''), fontsize=16, fontweight='bold')
    axes = axes.flatten()
    ...
    for ax, model_folder in zip(axes, folders):
        ...
        # short title
        m = re.search(r'_mini_(\d+)_final$', model_folder)
        if m:
            short = f"n{m.group(1)}"
